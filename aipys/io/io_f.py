...
def write_xyz(xyz, file_path, header=True):
...
def write_pc(xyz, file_path, charge_dict, header=True):
    with open(file_path, 'w') as file:
        if header:
            file.write(str(xyz.shape[0]) + '\n')
            file.write('\n')
...
def write_ecp(xyz, file_path, charge_dict, header=True):
    with open(file_path, 'w') as file:
        if header:
            file.write(str(xyz.shape[0]) + '\n')
            file.write('\n')
...
def write_constraints(constraints, file_path):
    with open(file_path, 'w') as file:
